...
The Power Management Extension introduces a set of functions that allow the
Supervisor to perform per-hart power management operations.
...
*priv* is a pointer to a opaque data segment that the caller can use to pass
...
Suspends the calling hart to a particular *power_state*, which is a
platform-specific value, outside the scope of this specification. Suspended hart
may automatically wake-up based on some wakeup event or trough a call to
`sbi_hart_up()`. After wake-up the hart will resume its operation on the
physical address pointed out by *resume_addr* in case of a wakeup event, or on
*start_addr* in case of `sbi_hart_up()`.

*priv* is a private data the caller can use to pass information about execution
context. The SBI implementation must save a copy so that caller can reuse while
restoring hart from suspend.
...
Returns the current status of *hartid* on sbiret->value, or an error through
