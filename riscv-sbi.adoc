...
Every SBI function should prefer `unsigned long` as the data type if possible.
It keeps the specification simple and easily adaptable for all RISC-V
ISA types (i.e. RV32, RV64 and RV128). In case the data is defined as 32bit wide,
the upper 32 bits should be considered as reserved bits unless otherwise specified.

If a SBI function need to pass a list of harts to the higher privilege mode, it
must use hart mask as defined below. This is applicable to any extensions defined
in or after v0.2.
...
If a lower privilege mode need to pass information about more than XLEN harts, it
should invoke multiple instance of the SBI function call.

Any function using hart mask may return following possible error values in addition
...
This is a replacement of legacy timer extension (0x00). It follows the new calling
...
This extension replaces the IPI related legacy extension (0x03). All the functions
follow the `hart_mask` defined in binary encoding section.
...
This extension replaces the remote fence related legacy extensions (0x05-0x07).
All the functions follow the `hart_mask` as defined in binary encoding section.
...
covering the range of guest physical addresses between start and size.  This covers
only the given VMID. This function call is only valid for harts implementing hypervisor
extension.
...
struct sbiret sbi_remote_hfence_vvma_asid(const unsigned long hart_mask,
				unsigned long hart_mask_base,
                                ...
                                unsigned long asid)
----
Instruct the remote harts to execute one or more HFENCE.VMA instructions,
covering the range of guest virtual addresses between start and size. This covers
only the given guest ASID.This function call is only valid for harts implementing hypervisor
extension.
...
| sbi_remote_hfence_vvma_asid   |           4 |   0x52464E43
|===
